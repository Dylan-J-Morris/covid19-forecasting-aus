...
def predict_plot(samples, df, third_date_range=None, third_omicron_date_range=None, 
                 split=True, moving=True, grocery=True,
                 R=2.2, md_arg=None, R_I=None,
                 ban='2020-03-16', var=None,
                 rho=None, prop=None, masks_prop=None,
                 second_phase=False, third_phase=False, 
                 third_states=None, prop_omicron_to_delta=None):
    """
    ...
    for i, state in enumerate(states):
        ...
        if split:
            ...
            if third_phase and states_initials[state] in third_states:
                ...
                delta_ve_samples = samples_sim[["ve_delta." + str(j+1) for j in range(third_days_tot)]].T
                omicron_ve_samples = samples_sim[["ve_omicron." + str(j+1) for j in range(third_omicron_days_tot)]].T
                delta_ve_tmp = delta_ve_samples.iloc[vax_idx_ranges[states_initials[state]],:]
                omicron_ve_tmp = omicron_ve_samples.iloc[omicron_ve_idx_ranges[states_initials[state]],:]
...
def plot_adjusted_ve(data_date, 
                     samples_mov_gamma,
                     states, 
