...
data {
    ...
    // number of months used for the third wave fitting
    int third_num_months;
...
parameters {
    ...
    positive_ordered[third_num_months] phi_unscaled; 
}
...
transformed parameters {
    ...
    // normalise the susceptibility factors
    simplex[third_num_months] phi = phi_unscaled / sum(phi_unscaled);
    ...
    for (i in 1:j_third){
        ...
        for (n in 1:N_third){
            if (include_in_third[i][n] == 1){
                md_third[pos] = pow(
                    1 + theta_md, -1 * prop_md_third[pos]
                );
                
                masks_third[pos] = pow(
                    1 + theta_masks, -1 * prop_masks_third[pos]
                );
                ...
                sus_dep_comp[1] = phi[1] * prop_inf_30[n,i];
                sus_dep_comp[2] = phi[2] * prop_inf_60[n,i];
                sus_dep_comp[3] = phi[3] * prop_inf_90[n,i];
                sus_dep_comp[4] = phi[4] * prop_inf_120[n,i];
