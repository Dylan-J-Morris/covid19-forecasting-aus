...
function initialise_population!(forecast::Forecast, start_day, D0)
    """
    ...
    for sim in 1:size(Z, 3)
        ...
        if D0.S == 0
            num_symptomatic_undetected = sample_negative_binomial_limit(
                1, p_detect_given_symp
            )
        else
            num_symptomatic_undetected = sample_negative_binomial_limit(
                D0.S, p_detect_given_symp
            )
        end
        ...
        if total_symptomatic == 0
            ...
            num_asymptomatic_undetected = sample_negative_binomial_limit(
                total_symptomatic, p_symp
            )
        end

        # total initial cases include the undetected
        total_initial_cases_sim =
            total_initial_cases + num_symptomatic_undetected + num_asymptomatic_undetected
        ...
        if total_initial_cases_sim > 0
            ...
            for _ in 1:(D0.S)
                Z[
                    infection_times[infection_time_counter] + 36, individual_type_map.S, sim
                ] += 1
                infection_time_counter += 1
            ...
            for _ in 1:(D0.A)
                Z[
                    infection_times[infection_time_counter] + 36, individual_type_map.A, sim
                ] += 1
                infection_time_counter += 1
...
function import_cases_model!(forecast::Forecast, import_cases, forecast_start_date)
    """
    ...
    for (i, t) in enumerate(forecast_days_plus_padding)
        ...
        if t <= T_observed
            ...
            current_ema =
                forecast.sim_constants.ϕ * count_on_day +
...
function sample_offspring!(
    ...
    k =
        forecast.sim_constants.k_delta * (t < forecast.sim_features.omicron_dominant_day) +
    ...
    if sum(Z_tmp) > 0
        ...
        TP_local_parent = max(
            0, TP_local_sim[TP_ind] * (1 - susceptible_depletion_sim * proportion_infected)
        )
        TP_import_parent = max(
            0, TP_import_sim[TP_ind] * (1 - susceptible_depletion_sim * proportion_infected)
        )
        ...
        if num_offspring > 0
            ...
            num_symptomatic_offspring_undetected =
                num_symptomatic_offspring - num_symptomatic_offspring_detected
            num_asymptomatic_offspring_undetected =
                num_asymptomatic_offspring - num_asymptomatic_offspring_detected
...
function simulate_branching_process(
    ...
    forecast = Forecast(sim_features, sim_realisations, sim_constants, individual_type_map)
    ...
    initialise_population!(forecast, start_day, D0)

    import_cases_model!(forecast, import_cases, forecast_start_date)
    ...
    proportion_infected = zeros(Int, size(TP_local_good_truncated, 1))

    # adjust the local TP by the number of realised cases 
    for (i, TP_ind) in enumerate(good_TPs_inds)
        proportion_infected = Z_good_cumulative[:, i] ./ N
        TP_local_good_truncated[:, i] =
            TP_local_good_truncated[:, i] .*
            (1 .- susceptible_depletion[TP_ind] * proportion_infected)
    end
