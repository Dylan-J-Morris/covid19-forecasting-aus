...
function read_in_TP(date, state)
    ...
    # get the dates 
    TP_dict_local["date"] = @pipe Matrix(_)[:](
        select(_, :date)(filter(:type => ==("R_L"), _)(filter(:state => ==(state), _)(df)))
    )
    ...
    # get the dates 
    TP_dict_import["date"] = @pipe Matrix(_)[:](
        select(_, :date)(filter(:type => ==("R_L"), _)(filter(:state => ==(state), _)(df)))
    )
    # create a vector of the indices
    ...
    for state in unique_states
        ...
        df_state_matrix = @pipe Matrix(_)(
            select(_, Not(1:8))(
                filter(:type => ==("R_L"), _)(filter(:state => ==(state), _)(df))
            ),
        )
        ...
        df_state_matrix = @pipe Matrix(_)(
            select(_, Not(1:8))(
                filter(:type => ==("R_I"), _)(filter(:state => ==(state), _)(df))
            ),
        )
