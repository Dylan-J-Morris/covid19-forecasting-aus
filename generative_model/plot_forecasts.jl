...
function plot_all_forecasts(
    file_date, states_to_plot, local_case_dict; zoom=false, confidence_level="both"
)
    """
    ...
    l = @layout [grid(8, 2; heights=heights_col)]
    ...
    for (i, state) in enumerate(states_to_plot)
        ...
        onset_dates_lims =
            Dates.value.([onset_dates[end] - Dates.Day(75), onset_dates[end]])
        ...
        hline!(fig; subplot=tp, [1], lc=:black, ls=:dash, label=false)

        vline!(fig; subplot=c, [Dates.Date(file_date)], lc=:gray, ls=:dash, label=false)

        vline!(fig; subplot=tp, [Dates.Date(file_date)], lc=:gray, ls=:dash, label=false)
...
function plot_single_forecast(file_date, state, local_case_dict; confidence_level="both")
    """
    Plots all the forecasts and saves the result as a pdf. 
    """
    ...
    TP_all_states = CSV.read(
        "results/UoA_forecast_output/2022-01-04/UoA_TP_" * file_date * ".csv", DataFrame
    )
    ...
    # define a custom 8x2 layout with the chosen heights 
    l = @layout [grid(2, 1; heights=heights)]

    # initialise the plot 
    fig = plot(;
        legend=false, layout=l, dpi=144, size=(700, 1200), link=:x, framestyle=:box
    )
    ...
    annotate!(fig; subplot=c, (0.2, 0.8), text(state, 8))
    ...
    hline!(fig; subplot=tp, [1], lc=:black, ls=:dash)
