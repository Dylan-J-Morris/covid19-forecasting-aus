function read_in_cases(date, rng; apply_inc=false, omicron_dominant_date=nothing)
    ...
    complete_dates[.!is_confirmation] =
        Date.(convert(Vector, df.date_onset[.!is_confirmation]))
    ...
    if apply_inc
        ...
        if apply_omicron
            ...
            inc_omicron = rand(
                rng, Gamma(shape_inc_omicron, scale_inc_omicron), length(complete_dates)
            )
    ...
    dates_since_start = range(Date("2020-03-01"), maximum(complete_dates); step=Day(1))
    ...
    for s in unique(state)
        ...
        local_cases =
            sum.(@views complete_dates_local_tmp .== dss for dss in dates_since_start)
        import_cases =
            sum.(@views complete_dates_import_tmp .== dss for dss in dates_since_start)
