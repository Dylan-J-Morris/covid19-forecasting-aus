...
function plot_all_forecast_intervals(file_date, states, local_case_dict)
    ...
    plot_all_forecasts(
        file_date, states_to_run, local_case_dict; zoom=true, confidence_level="both"
    )
    ...
    merge_pdfs(pdf_filenames, dir_name * file_name_tmp * file_date * ".pdf"; cleanup=true)
...
function simulate_single_state(file_date, state, nsims)
    ...
    onset_dates =
        latest_onset_date:Dates.Day(1):(latest_onset_date + Dates.Day(size(D, 1) - 1))
...
function visualise_data()
    ...
    # states to simulate 
    states_to_run = ["NSW", "QLD", "SA", "TAS", "VIC", "WA", "ACT", "NT"]
