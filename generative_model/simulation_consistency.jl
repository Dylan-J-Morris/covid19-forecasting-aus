...
function get_simulation_limits(
    ...
    sim_features = Features(
        max_forecast_cases, cases_pre_forecast, N, T_observed, T_end, omicron_dominant_day
    )

    window_lengths = 0

    return (sim_features, window_lengths, min_cases, max_cases)
end

function count_cases_in_windows!(
    forecast::Forecast, case_counts, window_lengths, D_total, sim
)
    """
...
function check_sim!(
    ...
    if !bad_sim && reinitialise_allowed
        ...
        if actual_3_day_cases > sim_3_day_threshold ||
            ...
            injected_cases = true
            inject_cases!(forecast::Forecast, missing_detections, day, sim)
        end
    end
...
function inject_cases!(forecast::Forecast, missing_detections, day, sim)
    """
    ...
    for _ in 1:num_symptomatic_detected
        ...
        infection_time =
            onset_time - sample_onset_time(; omicron=onset_time >= omicron_dominant_day)
