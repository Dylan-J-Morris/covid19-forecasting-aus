    ...
    for (i in 1:j_third_wave){
        ...
        for (n in 1:N_third_wave){
            if (include_in_third_wave[i][n]==1){
                ...
                // the total vaccine effect is the linear combination of no effect and the inferred vaccine effect
...
model {
    int pos2;
    // these parameters are defined to more easily allocate the 
    // prior on the vaccine effect
    real a_vacc; 
    real b_vacc;
    real vacc_sig = 0.001;
    real vacc_mu;
    ...
    for (i in 1:j_third_wave){
        ...
        for (n in 1:N_third_wave){
            if (include_in_third_wave[i][n]==1){
                ...
                // calculate shape and scale for Beta prior assuming the mean is the vaccine data
                // this is fine to do as none of the vaccine data used is close to 1 or 0 but would be an issue if 
                // waning becomes a prominent thing or if we get a vaccine resitant variant 
