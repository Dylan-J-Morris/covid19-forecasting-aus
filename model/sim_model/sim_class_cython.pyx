...
cdef class Forecast:
    ...
    cdef np.int_t num_of_sim, num_bad_sims, num_too_many, end_time
    cdef infected_queue
    cdef Reff_all     # this is a deque so cant state type
    cdef np.int_t max_cases
    ...
    cdef np.int_t data_check(self, day):
        """
